...
void draw_not_auth_prompt() {
	ImGui::Begin("CeaiHack Auth", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar);
	{
		...
		ImGui::BeginChildFrame(1, ImVec2(0.0f, -27.0f));
		{
			...
			ImGui::PopFont();
			ImGui::TextWrapped("CeaiHack is running in an UNAUTHORIZED state!\n\nThis means that it was either injected with another injector than ours, you are trying to reverse/tamper with our cheat or there is an unknown error.\n\nPlease try re-injecting CeaiHack again. If this issue still occurs, please contact the CeaiHack development team.\n\nThank you!"
			);
		}
		ImGui::EndChildFrame();
...
void draw() {
#ifdef _DEBUG
	if (!ceaihack::config::is_running) return;
...
void ceaihack::cheat::ui::on_frame(HDC context) {
	...
	if (ceaihack::config::features::aimassist::enabled && ceaihack::config::features::aimassist::show_prediction) {
		...
		bool in_corr_radius = current_radius <= ceaihack::cheat::features::aimassist::object_radius * ceaihack::config::features::aimassist::radius_start_correction && current_radius > ceaihack::cheat::features::aimassist::object_radius;

		char tmp[512] = { 0 };
		sprintf(tmp, "R:%.2f\nC:%.2f\nO%.2f", current_radius, ceaihack::cheat::features::aimassist::object_radius * ceaihack::config::features::aimassist::radius_start_correction, ceaihack::cheat::features::aimassist::object_radius);

		
		ImGui::GetOverlayDrawList()->AddCircleFilled(object_pos, ceaihack::cheat::features::aimassist::object_radius * ceaihack::config::features::aimassist::radius_start_correction, ImColor(0, 255, 255, 100)); // aim correction point - CYAN
		ImGui::GetOverlayDrawList()->AddCircleFilled(object_pos, ceaihack::cheat::features::aimassist::object_radius, ImColor(0, 255, 0, 100)); // object radius - GREEN

		ImGui::GetOverlayDrawList()->AddCircleFilled(ceaihack::cheat::features::aimassist::predicted_position.imvec() , 15, ImColor(100, 100, 100)); // cursor position - RED
