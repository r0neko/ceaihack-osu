...
void ceaihack::cheat::features::aimassist::perform_aim_correction(player* player, vector2_t pos) {
	cursor_position = pos;
	...
	if (time <= (objects[0].start_time.get() - 1500))
		...
		if (time <= (objects[0].start_time.get() - 1000))
			...
			for (int i = 0; i < objects.length; i++) {
				...
				if (time >= (object.start_time.get() - wait_time) && time <= object.end_time.get()) { // check if time is in the [start - preempt; end] interval.
					...
					cursor_position.interpolate_to(object_position, delta * config::features::aimassist::auto_interp_time);
					break;
				}
			}
		}
...
void __fastcall ceaihack::cheat::features::aimassist::set_cursor_position_hook(void* ecx, void* edx, vector2_t pos) {
	...
	if (ceaihack::config::features::aimassist::enabled && ceaihack::osu::player::is_instance()) {
		...
		if (player->hom->beatmap != nullptr && player->hom->hit_objects.is_valid() && player->hom->hit_objects.length > 0) {
			if (!ceaihack::config::features::aimassist::auto_aim) perform_aim_correction(player.get(), pos);
			else perform_auto_aim(player.get(), pos);
		}
