...
void ceaihack::cheat::features::unmod::flashlight::update() {
	...
	if ((ceaihack::memory::location::check_flashlight_hax == NULL || ceaihack::memory::location::update_flashlight == NULL) && ceaihack::osu::player::is_instance()) {
	...
	if (ceaihack::memory::location::check_flashlight_hax != NULL && ceaihack::hooks::variables::o_check_flashlight_hax == nullptr) {
		if (MH_CreateHook(ceaihack::memory::location::check_flashlight_hax, reinterpret_cast<void*>(&check_flashlight_hax_hook), reinterpret_cast<void**>(&ceaihack::hooks::variables::o_check_flashlight_hax)) != MH_OK) {
	...
	if (ceaihack::memory::location::update_flashlight != NULL && ceaihack::hooks::variables::o_update_flashlight == nullptr) {
		if (MH_CreateHook(ceaihack::memory::location::update_flashlight, reinterpret_cast<void*>(&update_flashlight_hook), reinterpret_cast<void**>(&ceaihack::hooks::variables::o_update_flashlight)) != MH_OK) {
