...
bool ceaihack::hooks::init() {
	...
	if (MH_CreateHook(reinterpret_cast<void*>(ceaihack::hooks::variables::wgl_swap_buffers), reinterpret_cast<void*>(&ceaihack::hooks::swap_buffers), reinterpret_cast<void**>(&ceaihack::hooks::variables::original_swap_buffers)) != MH_OK) {
		ceaihack::logger::cheat->error("Failed to hook wglSwapBuffers!");
		return 0;
	}

	if (MH_CreateHook(reinterpret_cast<void*>(ceaihack::memory::location::set_rate), reinterpret_cast<void*>(&ceaihack::cheat::features::timewarp::rate_change_hook), reinterpret_cast<void**>(&ceaihack::hooks::variables::o_set_current_playback_rate)) != MH_OK) {
		ceaihack::logger::cheat->error("Failed to hook set_CurrentPlaybackRate!");
	}

	if (MH_CreateHook(reinterpret_cast<void*>(ceaihack::memory::location::set_cursor_pos), reinterpret_cast<void*>(&ceaihack::cheat::features::aimassist::set_cursor_position_hook), reinterpret_cast<void**>(&ceaihack::hooks::variables::o_set_cursor_position)) != MH_OK) {
